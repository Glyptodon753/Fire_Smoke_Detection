...
def runCNN(dataset, input_shape, filters, dense, output_shape, epochs):
    ...
    plot_accuracy_loss(record.history, False)

    cm = confusion_matrix(model, dataset, dataset.validation_set, dataset.classes)
    plot_confusion_matrix(cm, False, dataset.classes)
    cm = cm / cm.sum(axis=1)[:, np.newaxis]
    plot_confusion_matrix(cm, True, dataset.classes)
